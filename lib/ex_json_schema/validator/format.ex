defmodule ExJsonSchema.Validator.Format do
  ...
  @date_time_regex ~r/^(-?(?:[1-9][0-9]*)?[0-9]{4})-(1[0-2]|0[1-9])-(3[01]|0[1-9]|[12][0-9])[tT](2[0-3]|[01][0-9]):([0-5][0-9]):([0-5][0-9])(\.[0-9]+)?([zZ]|[+-](?:2[0-3]|[01][0-9]):[0-5][0-9])?$/
  ...
  defp validate_with_regex(data, regex, failure_message_fun) do
    if Regex.match?(regex, data) do
      ...
      [%Error{error: %Error.Format{expected: failure_message_fun.(data)}, path: ""}]
    end
  end
end
