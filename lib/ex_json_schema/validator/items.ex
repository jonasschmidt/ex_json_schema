defmodule ExJsonSchema.Validator.Items do
  ...
  defp validate_items(_root, {[], false}, items, {errors, index}) do
    [%Error{error: %Error.AdditionalItems{additional_indices: index..(index + Enum.count(items) - 1)}, path: ""} | errors]
  end
  defp validate_items(root, {[], additional_items_schema}, [item | items], {errors, index}) do
    acc = {[Validator.validation_errors(root, additional_items_schema, item, "/#{index}") | errors], index + 1}
    validate_items(root, {[], additional_items_schema}, items, acc)
  end
  defp validate_items(root, {[schema | schemata], additional_items}, [item | items], {errors, index}) do
