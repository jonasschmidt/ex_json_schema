defmodule NExJsonSchema.ConfigTest do
  use ExUnit.Case

  import NExJsonSchema.Schema, only: [resolve: 1]
  ...
  test "raising an exception when trying to resolve a remote schema and no remote schema resolver is defined" do
    Application.put_env(:nex_json_schema, :remote_schema_resolver, nil)
    assert_raise NExJsonSchema.Schema.UndefinedRemoteSchemaResolverError, fn -> resolve(@schema) end
  end

  test "defining a remote schema resolver with module and function name" do
    Application.put_env(:nex_json_schema, :remote_schema_resolver, {NExJsonSchema.ConfigTest, :test_resolver})
